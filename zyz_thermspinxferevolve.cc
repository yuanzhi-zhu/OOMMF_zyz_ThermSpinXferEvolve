/* FILE: zyz_thermspinxferevolve.cc                 -*-Mode: c++-*-
 *
 * Concrete evolver class, built on top of the Oxs_RungeKuttaEvolve
 * class, but including a spin injection field torque.
 *
 * zyz_ThermSpinXferEvolve is a variant of Oxs_SpinXferEvolve written by
 * Yuanzhi Zhu, which adds support for non-zero temperatures and space varying temperatures.
 *
 * zyz_ThermSpinXferEvolve version date: 1 March 2020
 * Requires OOMMF 1.2a4pre 20091216 or later.
 *
 * This is the 2.0 version of zyz_thermspinxferevolce, in the version 1.0,
 * I try to modify the varible temperature from OC_REAL8m to Oxs_MeshValue<OC_REAL8m>,
 * but it did not wrok for unknow reason. In the 2.0 version I simply add a new varible
 * Therm which is an Oxs_MeshValue<OC_REAL8m> data, what I did is multiply this Therm
 * with the field to make the temperature field a space-varing one. 
 * 
 * Besides, I also add a corresponding therm-field output, which is exactly Therm times the
 * Gaussian field and input temperature. I also modify the temperature as the spatial average
 * temperature.
 * 
 * Also, during the days I try to add the space-varing function, I wrote some comments
 * in this file in order to make this file more readible.
 */

// The following four head files are cpp standard libraries
#include <float.h>
#include <string>
#include <stdlib.h>
#include <time.h>

// Following head files are oommf head files
#include "nb.h"
#include "director.h"
#include "timedriver.h"
#include "simstate.h"
#include "zyz_thermspinxferevolve.h"
...
OXS_EXT_REGISTER(zyz_ThermSpinXferEvolve);

/* End includes */

OC_REAL8m zyz_ThermSpinXferEvolve::EvaluateTemperatureProfileScript
(OC_UINT4m stage,
...
// Constructor
zyz_ThermSpinXferEvolve::zyz_ThermSpinXferEvolve(
  const char* name,     // Child instance id
  Oxs_Director* newdtr, // App director
  const char* argstr)   // MIF input block parameters
  // inheritance from the TimeEvolver
    ...
    rkstep_ptr(NULL)
{
  ...
  if(P_free_init.GetPtr()==NULL) {
    ...
    /// except as part of zyz_ThermSpinXferEvolve instance
  ...
  // Lambda: used to calculate epsilon: coef of first spin transfer term
  ...
  // eps_prime: coef of second spin transfer term
  ...
  // get value for alpha, 0.5 by default
  ...
  /// This is the additional varible which achieves the space-varing function
  /// The default value is a uniform uniform with magnitude 1, namely do nothing
  if(HasInitValue("Therm")) {
    /// Function OXS_GET_INIT_EXT_OBJECT 'macro' is VERY important for that it's the bridge
    /// that connect the input string and the real object, namely it processes arguments
    /// in the mif file, as a result Get scalar field object
    OXS_GET_INIT_EXT_OBJECT("Therm",Oxs_ScalarField,Therm_init);
  } else {
    Therm_init.SetAsOwner(dynamic_cast<Oxs_ScalarField *>
                          (MakeNew("Oxs_UniformScalarField",director,
                                   "value 1")));
  }
  ...
  // Why here is for temperatre > 0?
  // reason:--> originally the default temperature is 0 Kelvin, and this means that
  // you can only use method "rk2heun" for temperature larger than 0
  ...
  if(temperature>0.) {
    ...
    if(method.compare("rk2heun")==0) {
      rkstep_ptr = &zyz_ThermSpinXferEvolve::TakeRungeKuttaStep2Heun;
    } else {
      throw Oxs_ExtError(this,"Invalid initialization detected:"
    ...
    if(method.compare("rk2")==0) {
      rkstep_ptr = &zyz_ThermSpinXferEvolve::TakeRungeKuttaStep2;
    } else if(method.compare("rk2heun")==0) {
      rkstep_ptr = &zyz_ThermSpinXferEvolve::TakeRungeKuttaStep2Heun;
    } else if(method.compare("rk4")==0) {
      rkstep_ptr = &zyz_ThermSpinXferEvolve::TakeRungeKuttaStep4;
    } else if(method.compare("rkf54m")==0) {
      rkstep_ptr = &zyz_ThermSpinXferEvolve::TakeRungeKuttaFehlbergStep54M;
    } else if(method.compare("rkf54s")==0) {
      rkstep_ptr = &zyz_ThermSpinXferEvolve::TakeRungeKuttaFehlbergStep54S;
    } else if(method.compare("rkf54")==0) {
      rkstep_ptr = &zyz_ThermSpinXferEvolve::TakeRungeKuttaFehlbergStep54;
    } else {
      throw Oxs_ExtError(this,"Invalid initialization detected:"
			      " \"method\" value must be one of"
  ...
  max_dm_dt_output.Setup(this,InstanceName(),"Max dm/dt","deg/ns",0,
     &zyz_ThermSpinXferEvolve::UpdateDerivedOutputs);
  dE_dt_output.Setup(this,InstanceName(),"dE/dt","J/s",0,
     &zyz_ThermSpinXferEvolve::UpdateDerivedOutputs);
  delta_E_output.Setup(this,InstanceName(),"Delta E","J",0,
     &zyz_ThermSpinXferEvolve::UpdateDerivedOutputs);
  temperature_output.Setup(this,InstanceName(),"Average-temperature","K",0,
     &zyz_ThermSpinXferEvolve::UpdateTemperatureOutputs);
  ave_J_output.Setup(this,InstanceName(),"average J","A/m^2",0,
     &zyz_ThermSpinXferEvolve::UpdateSpinTorqueOutputs);

  Therm_output.Setup(this,InstanceName(),"Thermal-field","",0,
     &zyz_ThermSpinXferEvolve::UpdateTemperatureOutputs);
  ...
  Therm_output.Register(director,-5);
...
///////////////////////////////////
// contain the temperature(Therm) part
OC_BOOL zyz_ThermSpinXferEvolve::Init()
{
...
void zyz_ThermSpinXferEvolve::UpdateMeshArrays(const Oxs_Mesh* mesh)
{
  ...
  if(rmesh==NULL) {
    String msg="Import mesh to zyz_ThermSpinXferEvolve::UpdateMeshArrays"
      " is not an Oxs_RectangularMesh object.";
    throw Oxs_ExtError(msg.c_str());
  }

  // define cell_thickness to derive the value of factor beta_factor
  ...
  //fabs() are used for calculating the absolute value of floating type numbers.
  OC_REAL8m beta_factor = fabs(hbar/(MU0*e_charge));
  if(cell_thickness>0.) beta_factor /= cell_thickness;
  else                  beta_factor = 0.0; // Safety

  /// FillMeshValue is a kind of functions which are defined in files like scalarfield.cc, vectorfield.cc and so on
  /// FillMeshValue assign values (initial values for here), to 'Oxs_MeshValue<OC_REAL8m>& array'(second parameter)
  ...
  OC_INDEX i,j;
  ave_J = 0.0;
  // the next two loop are calculating for the current J
  for(i=0;i<size;i++) {
    ave_J += A[i];
    ...
    if(fixed_temp<1.0) {
      char buf[1024];
      Oc_Snprintf(buf,sizeof(buf),"zyz_ThermSpinXferEvolve::UpdateMeshArrays:"
  ...
  /// Similar to FillMeshValue, MultMeshValue multiply each mesh cell a value instead of assign a value
  ...
  Therm_init->FillMeshValue(mesh,Therm);
...
// Propagate_mp is totally the same as in the cpp file spinxferevolve
void
zyz_ThermSpinXferEvolve::Propagate_mp
(const Oxs_Mesh* mesh,
 const Oxs_MeshValue<ThreeVector>& spin)
{
  ...
  if(!mp.CheckMesh(mesh)) {
    throw Oxs_ExtError("mp array not properly initialized"
                         " in zyz_ThermSpinXferEvolve::Propagate_mp");
  }

  const Oxs_RectangularMesh* rmesh
    = dynamic_cast<const Oxs_RectangularMesh*>(mesh);
  if(rmesh==NULL) {
    String msg="Import mesh to zyz_ThermSpinXferEvolve::Propagate_mp"
      " is not an Oxs_RectangularMesh object.";
    throw Oxs_ExtError(msg.c_str());
  }
...
// This part is totally the same as in the cpp file spinxferevolve
OC_REAL8m zyz_ThermSpinXferEvolve::EvaluateJProfileScript
(OC_UINT4m stage,
 OC_REAL8m stage_time,
 OC_REAL8m total_time) const
{
...
// This part is totally the same as in the cpp file spinxferevolve
OC_REAL8m
zyz_ThermSpinXferEvolve::PositiveTimestepBound
(OC_REAL8m max_dm_dt)
...
/// This is where we update the d_{ms} for the calcualtion in the final LLG eq.
void zyz_ThermSpinXferEvolve::Calculate_dm_dt
  ...
  */
  ...
  for(i=0;i<size;i++) {
    if(Ms_[i]==0) {
      ...
      /////////////////////////////////////////////
      /// Add Therm
      /// we define this as OC_REAL8m varible cell_Therm because we only
      /// need to use it for each mesh cell inside the loop
      const OC_REAL8m cell_Therm = Therm[i];
      ...
      ThreeVector scratchA = mxH;  /// use scratchA to store mxH
      ...
      /// h_fluctVarConst stores 2*kB*T*alpha/((1+alpha^2)*gamma*MU0*Vol)
      if(temperature>0.) {
	      tmpHfluct = Hfluct[i];         // Standard normal random variable (eta)
        /// I donot like the function which generate h_fluctVarConst, so I change it into the following line
        /// What different is that the temperature is replaced with temperature*Therm[i]
        
        /// h_fluctVarConst is proportional to the temperature, which means that the higher the temperature, 
        /// the stronger the fluctuation
        OC_REAL8m h_fluctVarConst_zyz = 2*fabs(alpha[i])/(1+alpha[i]*alpha[i])/(MU0*fabs(gamma[i])*min_timestep)*KBoltzmann*temperature*Therm[i]/mesh->Volume(i);
          
        tmpHfluct *= sqrt(h_fluctVarConst_zyz*Ms_inverse_[i]*Tmult); // eta*sqrt(2*kB*T*alpha/((1+alpha^2)*gamma*MU0*Ms*Vol*dt)
        //tmpHfluct *= sqrt(h_fluctVarConst[i]*Ms_inverse_[i]*Tmult); // eta*sqrt(2*kB*T*alpha/((1+alpha^2)*gamma*MU0*Ms*Vol*dt)
        mxHfluct ^= tmpHfluct;  /// What's the purpose of this '^' operator --> This operater is defined for the data structure as cross multiply
        scratchA += mxHfluct;
      }
...
// This part is totally the same as in the cpp file spinxferevolve
void zyz_ThermSpinXferEvolve::CheckCache(const Oxs_SimState& cstate)
{
  ...
  if(!cache_good) {
    throw Oxs_ExtError(this,
       "zyz_ThermSpinXferEvolve::CheckCache: Invalid data cache.");
  }
}
...
// This part is totally the same as in the cpp file spinxferevolve
void
zyz_ThermSpinXferEvolve::AdjustState
(OC_REAL8m hstep,
 OC_REAL8m mstep,
 ...
 OC_REAL8m& norm_error) const
{
  ...
  // to zyz_ThermSpinXferEvolve::Step() takes 2 half-steps, and it is the
  ...
  // in by the zyz_ThermSpinXferEvolve::NegotiateTimeStep() method.
}


// This part is totally the same as in the cpp file spinxferevolve
void zyz_ThermSpinXferEvolve::UpdateTimeFields
...
// This part is totally the same as in the cpp file spinxferevolve
void zyz_ThermSpinXferEvolve::NegotiateTimeStep
...
{ // This routine negotiates with driver over the proper step size.
  ...
  if(!cstate.GetDerivedData("Max dm/dt",max_dm_dt)) {
    throw Oxs_ExtError(this,
       "zyz_ThermSpinXferEvolve::NegotiateTimeStep: max_dm_dt not cached.");
  }
  ...
  if(!cstate.GetDerivedData("Timestep lower bound",
                            timestep_lower_bound)) {
    throw Oxs_ExtError(this,
       "zyz_ThermSpinXferEvolve::NegotiateTimeStep: "
...
// This part is totally the same as in the cpp file spinxferevolve
OC_BOOL
zyz_ThermSpinXferEvolve::CheckError
...
// in the next few functions, several new lines are added for each of them, about the temperature field
// Comparing with cpp file spinxferevolve, a new line 'tmpStepsize = stepsize;' is added
void zyz_ThermSpinXferEvolve::TakeRungeKuttaStep2
(OC_REAL8m stepsize,
...
{ // This routine performs a second order Runge-Kutta step, with
  ...
  if(!endstate->AddDerivedData("Timestep lower bound",
    ...
    throw Oxs_ExtError(this,
                         "zyz_ThermSpinXferEvolve::TakeRungeKuttaStep2:"
...
/// Since I need to simulate the situation with temperature large than 0, only need to modify this function!
/// Comparing with cpp file spinxferevolve, four new lines are added

/// In this method, the only difference is that we update the Hfluct[i] field and use a different
/// ``Calculate_dm_dt`` function(the same name with more content)
void zyz_ThermSpinXferEvolve::TakeRungeKuttaStep2Heun
(OC_REAL8m stepsize,
 Oxs_ConstKey<Oxs_SimState> current_state_key,
 const Oxs_MeshValue<ThreeVector>& current_dm_dt,
 Oxs_Key<Oxs_SimState>& next_state_key,
...
{ // This routine performs a second order Runge-Kutta step, with
  ...
  if(!endstate->AddDerivedData("Timestep lower bound",
     ...
     !endstate->AddDerivedData("dE/dt",dE_dt)) {
    throw Oxs_ExtError(this,
                         "zyz_ThermSpinXferEvolve::TakeRungeKuttaStep2Heun:"
                         " Programming error; data cache already set.");
  }
...
void zyz_ThermSpinXferEvolve::TakeRungeKuttaStep4
(OC_REAL8m stepsize,
 Oxs_ConstKey<Oxs_SimState> current_state_key,
 const Oxs_MeshValue<ThreeVector>& current_dm_dt,
 Oxs_Key<Oxs_SimState>& next_state_key,
...
void zyz_ThermSpinXferEvolve::RungeKuttaFehlbergBase54
(RKF_SubType method,
 OC_REAL8m stepsize,
 Oxs_ConstKey<Oxs_SimState> current_state_key,
 const Oxs_MeshValue<ThreeVector>& current_dm_dt,
...
{ // Runge-Kutta-Fehlberg routine with combined 4th and 5th
  ...
  default:
    throw Oxs_ExtError(this,
                 "zyz_ThermSpinXferEvolve::RungeKuttaFehlbergBase54:"
...
void zyz_ThermSpinXferEvolve::TakeRungeKuttaFehlbergStep54
(OC_REAL8m stepsize,
 Oxs_ConstKey<Oxs_SimState> current_state_key,
 const Oxs_MeshValue<ThreeVector>& current_dm_dt,
 Oxs_Key<Oxs_SimState>& next_state_key,
...
void zyz_ThermSpinXferEvolve::TakeRungeKuttaFehlbergStep54M
(OC_REAL8m stepsize,
 Oxs_ConstKey<Oxs_SimState> current_state_key,
 const Oxs_MeshValue<ThreeVector>& current_dm_dt,
 Oxs_Key<Oxs_SimState>& next_state_key,
...
void zyz_ThermSpinXferEvolve::TakeRungeKuttaFehlbergStep54S
(OC_REAL8m stepsize,
 Oxs_ConstKey<Oxs_SimState> current_state_key,
 const Oxs_MeshValue<ThreeVector>& current_dm_dt,
 Oxs_Key<Oxs_SimState>& next_state_key,
...
// This part is totally the same as in the cpp file spinxferevolve
OC_REAL8m zyz_ThermSpinXferEvolve::MaxDiff
 ...
 const Oxs_MeshValue<ThreeVector>& vecB)
{
  ...
  if(vecB.Size()!=size) {
    throw Oxs_ExtError(this,
                 "zyz_ThermSpinXferEvolve::MaxDiff:"
...
// This part is totally the same as in the cpp file spinxferevolve
void zyz_ThermSpinXferEvolve::AdjustStepHeadroom(OC_INT4m step_reject)
...
// This part is totally the same as in the cpp file spinxferevolve
OC_BOOL
zyz_ThermSpinXferEvolve::Step(const Oxs_TimeDriver* driver,
                      Oxs_ConstKey<Oxs_SimState> current_state_key,
                      ...
                      Oxs_Key<Oxs_SimState>& next_state_key)
{
  ...
  if(next_timestep<=0.0 ||
    ...
    if(cstate.stage_number==0
      ...
      throw Oxs_ExtError(this,
           "zyz_ThermSpinXferEvolve::Step; Programming error:"
  ...
  // -mjd, 21-July-2004
  {
    ...
    if(!cstate.GetDerivedData("dE/dt",current_dE_dt)) {
      throw Oxs_ExtError(this,"zyz_ThermSpinXferEvolve::Step:"
...
/// the following several functions are about the fluctuation field,
/// which is induced by the temperature

/// Hfluct itself doesnot need to change
void zyz_ThermSpinXferEvolve::InitHfluct(const Oxs_Mesh* mesh)
{
  // initialize the Hfluct field as zero
  const OC_INDEX size = mesh->Size();
  for(OC_INDEX i=0;i<size;i++) {
    Hfluct[i].Set(0., 0., 0.);
  }
...
void zyz_ThermSpinXferEvolve::UpdateHfluct(const Oxs_Mesh* mesh)
{
  // Iterating through all the meshes and assign value to the HFluct field
  const OC_INDEX size = mesh->Size();
  OC_INDEX i, j;
  for(i=0;i<size;i++) {
...
/// Unused function
/*
void zyz_ThermSpinXferEvolve::FillHfluctConst(const Oxs_Mesh* mesh)
{
...
// This part is totally the same as in the cpp file spinxferevolve
void zyz_ThermSpinXferEvolve::UpdateDerivedOutputs(const Oxs_SimState& state)
...
// This part is totally the same as in the cpp file spinxferevolve
void zyz_ThermSpinXferEvolve::UpdateSpinTorqueOutputs(const Oxs_SimState& state)
{
...
// This functions is about the fluctuation field ouput
// Here I decide to output the unit vector temperature field: Therm itself instead of the original number temperature
void zyz_ThermSpinXferEvolve::UpdateTemperatureOutputs(const Oxs_SimState& state)
{
  const Oxs_Mesh* mesh = state.mesh;
  const OC_INDEX size = mesh->Size();
  OC_INDEX i;
  ...
  if(Therm_output.GetCacheRequestCount()>0) {
    Therm_output.cache.state_id = 0;

    Oxs_MeshValue<ThreeVector>& Tfield = Therm_output.cache.value;
    Tfield.AdjustSize(mesh);
    for(i=0;i<size;i++) {
      if(Ms[i]==0) {
        Tfield[i].Set(0.0,0.0,0.0);
      } else {
        Tfield[i] = Therm[i]*Hfluct[i];
        Tfield[i] *= Tmult*temperature;
      }
    }

    Therm_output.cache.state_id = state.Id();
  }
  ...
  if(temperature_output.GetCacheRequestCount()>0) {
    ...
    temperature_output.cache.value=ave_T*Tmult;
    temperature_output.cache.state_id = state.Id();
  }
  
}

// Gaussian random field created
// mu is the average value and sigma is the variance
OC_REAL8m zyz_ThermSpinXferEvolve::Gaussian_Random(const OC_REAL8m muGaus, const OC_REAL8m sigmaGaus){
// This function accept \mu and \sigma and return a ``number`` satisfied the 
